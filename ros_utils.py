...
import rosbag
from sensor_msgs.msg import JointState  # 関節角度のメッセージ型
from cv_bridge import CvBridge  # 画像データの変換用
...
class RGBD():
    ...
    def __init__(self):
        ...
        self._bridge = CvBridge()  # CvBridgeのインスタンスを追加

        # 関節角度データのサブスクライバ
        self._joint_sub = rospy.Subscriber(
            "/hsrb/joint_states", JointState, self._joint_cb)
    ...
    def save_data_to_bag(self, filename):
        u"""rosbagに画像情報と関節角度情報を保存する関数"""
        with rosbag.Bag(filename, 'w') as bag:
            # ポイントクラウドと画像データの保存
            if self._points_data is not None:
                bag.write('/hsrb/head_rgbd_sensor/depth_registered/rectified_points', self._points_data)
            if self._image_data is not None:
                # 画像データを対応するトピックに保存
                image_msg = self._bridge.cv2_to_imgmsg(self._image_data, encoding="rgb8")
                bag.write('/hsrb/head_rgbd_sensor/rgb_image', image_msg)
            
            # 関節角度データの保存
            if self._joint_states is not None:
                bag.write('/hsrb/joint_states', self._joint_states)