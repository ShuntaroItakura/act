...
def main(args):
    ...
    if is_eval:
        ...
        for ckpt_name in ckpt_names:
            success_rate, avg_return = eval_bc(val_dataloader,config, ckpt_name, save_episode=True)
...
def eval_bc(val_dataloader, config, ckpt_name, save_episode=True):
    set_seed(1000)
    ...
    with torch.inference_mode():
        for batch_idx, data in enumerate(val_dataloader):
            ...
            # バッチ内の各シーケンスを評価
            # for t in range(qpos_data.shape[1]):  # タイムステップ分ループ
            for t in range(2):
                qpos = qpos_data[:, t]
                print(qpos.shape,'qpos.aha')
                qpos = qpos_data
                curr_image = image_data
                actions = action_data[:, :t+1]
                actions = action_data
                ...
                raw_action = all_actions[:, t % all_actions.shape[1]]
                raw_action = raw_action.squeeze(0).cpu().numpy()

                # # アクションを適用し、報酬を記録
                # action = (raw_action * config['policy_config']['action_std'] +
            ...
            # episode_return = np.sum(batch_rewards)
            # episode_returns.append(episode_return)
            # highest_reward = np.max(batch_rewards)
            # highest_rewards.append(highest_reward)

    # # 成果の集計
    # success_rate = np.mean(np.array(highest_rewards) == config.get('env_max_reward', 1))
